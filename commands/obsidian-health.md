...
4. Spawn parallel subagents to handle each category simultaneously:
   Include in each agent's prompt: the vault root path and the JSON findings for its category. Agents must not re-run the health script or re-read `_CLAUDE.md` - everything they need is in their prompt.
